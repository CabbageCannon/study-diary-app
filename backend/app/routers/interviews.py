...
def require_admin(request: Request) -> None:
    if not request.state.is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="仅管理员可操作题库")
    ...
    if questions:
        answered_ids = set(
            db.scalars(
                ...
                .where(
                    InterviewAnswer.question_id.in_([question.id for question in questions]),
                    InterviewAnswer.user_id == request.state.user_id,
                )
                .distinct()
            ).all()
        )
...
def get_interview_batch_job(job_id: str, request: Request, db: Session = Depends(get_db)) -> InterviewBatchJobRead:
    require_admin(request)
    require_question_review_enabled()
...
def create_interview_question_set(
    ...
    try:
        question_set, message = create_question_set(db, payload, request.state.user_id)
        return get_question_set_read(db, question_set, request.state.user_id, message)
    except ValueError as exc:
...
def list_interview_question_sets(
    ...
    return list_question_set_summaries(db, limit, request.state.user_id, status=status_filter)


@router.get("/stats", response_model=InterviewTrainingStats)
def get_interview_training_stats(request: Request, db: Session = Depends(get_db)) -> InterviewTrainingStats:
    return get_training_stats(db, request.state.user_id)


@router.get("/question-sets/{question_set_id}", response_model=InterviewQuestionSetRead)
def get_interview_question_set(question_set_id: int, request: Request, db: Session = Depends(get_db)) -> InterviewQuestionSetRead:
    question_set = training_repository.get_question_set(db, question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    return get_question_set_read(db, question_set, request.state.user_id)
...
def update_interview_question_set_progress(
    ...
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        updated = update_question_set_progress(db, question_set, payload, request.state.user_id)
        return get_question_set_read(db, updated, request.state.user_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc


@router.post("/question-sets/{question_set_id}/complete", response_model=InterviewQuestionSetRead)
def complete_interview_question_set(question_set_id: int, request: Request, db: Session = Depends(get_db)) -> InterviewQuestionSetRead:
    question_set = training_repository.get_question_set(db, question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        completed = complete_question_set(db, question_set, request.state.user_id)
        return get_question_set_read(db, completed, request.state.user_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc


@router.post("/question-sets/{question_set_id}/abandon", response_model=InterviewQuestionSetRead)
def abandon_interview_question_set(question_set_id: int, request: Request, db: Session = Depends(get_db)) -> InterviewQuestionSetRead:
    question_set = training_repository.get_question_set(db, question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        abandoned = abandon_question_set(db, question_set, request.state.user_id)
        return get_question_set_read(db, abandoned, request.state.user_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
...
def restart_interview_question_set(question_set_id: int, request: Request, db: Session = Depends(get_db)) -> InterviewQuestionSetRead:
    question_set = training_repository.get_question_set(db, question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        restarted, message = restart_question_set(db, question_set, request.state.user_id)
        return get_question_set_read(db, restarted, request.state.user_id, message)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc


@router.delete("/question-sets/{question_set_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_interview_question_set(question_set_id: int, request: Request, db: Session = Depends(get_db)) -> None:
    question_set = training_repository.get_question_set(db, question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    delete_question_set(db, question_set, request.state.user_id)


@router.post("/question-sets/{question_set_id}/skip", response_model=InterviewQuestionSetRead)
def skip_interview_question(question_set_id: int, request: Request, db: Session = Depends(get_db)) -> InterviewQuestionSetRead:
    question_set = training_repository.get_question_set(db, question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        updated = skip_current_question(db, question_set, request.state.user_id)
        return get_question_set_read(db, updated, request.state.user_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
...
async def submit_interview_answer(
    ...
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        return submit_and_queue_evaluation(db, question_set, payload, background_tasks, request.state.user_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
...
async def retry_interview_answer(
    ...
    previous_answer = training_repository.get_answer(db, answer_id, request.state.user_id)
    ...
    question_set = training_repository.get_question_set(db, previous_answer.question_set_id, request.state.user_id)
    if question_set is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="训练题集不存在")
    try:
        return submit_and_queue_evaluation(db, question_set, payload, background_tasks, request.state.user_id, retry=True)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
...
async def evaluate_saved_interview_answer(
    ...
    answer = training_repository.get_answer(db, answer_id, request.state.user_id)
    if answer is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="回答不存在")
    return queue_retry_evaluation(db, answer, background_tasks, request.state.user_id)
...
def get_due_interview_reviews(
    ...
    try:
        return list_due_reviews(db, request.state.user_id, date=date, domain=domain, limit=limit)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
