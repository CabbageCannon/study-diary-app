...
export function DiaryPage() {
  ...
  return <div className="diary-page">
    ...
    {detail ? <div className="diary-overlay" role="presentation" onClick={() => setDetail(null)}><article aria-modal="true" className="diary-detail-sheet" onClick={(event) => event.stopPropagation()} role="dialog"><button aria-label="关闭日记" className="diary-sheet-close" onClick={() => setDetail(null)} type="button"><XIcon size={18} /></button>{detail.images.length ? <div className={`diary-detail-photos count-${detail.images.length}`}>{detail.images.map((image, index) => <img alt={`日记照片 ${index + 1}`} key={image.slice(-28)} src={image} />)}</div> : null}<div className="diary-detail-copy"><div className="diary-detail-meta"><span>{detail.category === "learning" ? "学习" : "生活"}</span><time>{diaryTimestamp(detail)}</time></div><div className="diary-detail-context"><MapPinIcon aria-hidden="true" size={13} />{[detail.location, detail.weather].filter(Boolean).join(" · ") || "地点与天气未记录"}</div><h2>{detail.title}</h2><p className="copyable-text">{detail.polished_text}</p></div></article></div> : null}
