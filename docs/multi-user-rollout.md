...
5. **认领旧记录。** 在具有 Render 环境变量的后端 Shell 中，先运行 `python scripts/claim_legacy_data.py --admin-user-id <UUID>` 查看各表待认领数量；与备份中的数量核对后，运行同一命令并加 `--apply`。脚本在单一事务内完成归属、校验，PostgreSQL 还会把这些列改为 `NOT NULL`。脚本可重复执行；再次运行待认领数量应全部为 0。
