...
2. **配置 Supabase Auth。** 开启 Email provider 和邮箱验证；当前没有自有发信域名，先用管理员 QQ 邮箱的 SMTP 授权码配置自定义 SMTP（`smtp.qq.com:465`，用户名和发件邮箱均为管理员邮箱），并实测邀请邮件、验证邮件和密码重置邮件。授权码只填在 Supabase 控制台，不写入仓库。将正式 Cloudflare 域名设为 Site URL，并加入 Redirect URLs。旧版项目默认 SMTP 只允许向授权邮箱发信，不能用于公开注册；日后有域名再切换到 Resend。
...
4. **建立管理员。** 保持公开注册关闭，在 Supabase Auth → Users 给你的邮箱发送邀请。打开邮件链接，在新版 App 设置密码并登录一次。在 Auth 用户列表复制该用户 UUID，设为 Render 的 `ADMIN_USER_ID` 并重新部署。再次登录后，「我的」应显示管理员入口。
